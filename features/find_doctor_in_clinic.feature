...
Scenario: User searches for a doctor with valid information
    ...
    And I am on the `Find a Doctor` page
    When I fill in `Name` with `John Simth`
    And I fill in `Speciality` with `Physical therapy`
    And I click `Search`
    Then I should see a doctor named `John Simth` specializing in `Physical therapy`
...
Scenario: User searches for a clinic with invalid information
    ...
    Given I've chosen a clinic already
    And I am on the `Find a Doctor` page
    When I fill in `Name' with a nonexsitent name
    When I click `Search`
